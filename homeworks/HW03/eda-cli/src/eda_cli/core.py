...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, high_cardinality_percent: float = 0.75) -> Dict[str, Any]:
    """
    ...
    # Проверяет, есть ли дубликаты user_id
    user_id_columns = [col for col in summary.columns if col.name == "user_id"]
    ...
    # Проверяет категориальные данные на большое количество уникальных значений
    flags["has_high_cardinality_categoricals"] = False
    for col in summary.columns:
        if not(col.is_numeric) and col.non_null > 0:
            # Уникальность относительно не-NaN значений
            uniqueness_ratio = col.unique / col.non_null
            if (uniqueness_ratio > high_cardinality_percent):
    ...
    if summary.n_cols > 100:
        score -= 0.1
    if flags["has_suspicious_id_duplicates"] is False:
        score -= 0.6
