...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> dict[str, dict[str, Union[bool, float]]]:
    """
    Эндпоинт, который принимает CSV-файл, запускает EDA-ядро
...
async def head(
    ...
    """
    HTTP версия CLI команды head
    Возвращает первые n строк CSV файла
    """
    ...
    try:
        df = pd.read_csv(file.file, sep=sep, encoding=encoding)
    except Exception as exc:
        raise HTTPException(status_code=400, detail=f"Не удалось прочитать CSV: {exc}")
    if df.empty:
        raise HTTPException(status_code=400, detail="CSV-файл не содержит данных (пустой DataFrame).")
