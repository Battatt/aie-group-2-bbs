...
### 5. `POST /quality_flags_from_csv` - получение  полного набора флагов качества датасета
...
- возвращает ассоциативный массив с ключом `flags`, по которому записан словарь всех флагов качества.

---

### 6. `POST /head` - получение нескольких первых записей датасета

Эндпоинт принимает CSV-файл, внутри:

- читает его в `pandas.DataFrame`;
- вызывает из `eda_cli.cli` функцию `get_head_data`
- возвращает ассоциативный массив - результат работы функции `get_head_data` с значениями первых нескольких строк данных
---
